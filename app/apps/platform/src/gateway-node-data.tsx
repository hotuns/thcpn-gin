    ...
    <Panel className="gateway-node-query">
      ...
      {nodesQuery.isLoading ? <StateView type="loading" title="正在加载节点" description="正在读取节点与指标。" /> : <>
        <div className="gateway-node-query-fields">
          <label className="field"><span className="field-label">查询方式</span><select value={mode} onChange={(event) => setMode(event.target.value as "node" | "compare")}><option value="node">节点数据</option><option value="compare">节点对比</option></select></label>
          {mode === "node" && <label className="field"><span className="field-label">节点</span><select value={selectedNode} onChange={(event) => selectNode(event.target.value)} disabled={!nodes.length}>{nodes.map((node) => <option key={node.key} value={node.key}>{node.name}</option>)}</select></label>}
        </div>
        <DeviceQueryToolbar range={range} onChange={setRange}/>
        {mode === "compare" ? <label className="field"><span className="field-label">对比指标</span><select value={selected[0] ?? ""} onChange={(event) => setSelected([event.target.value])}>{metrics.map((metric) => <option key={metric.key} value={metric.key}>{metric.name}{metric.unit ? ` · ${metric.unit}` : ""} · {metric.count} 个节点</option>)}</select></label> : <fieldset className="gateway-node-metric-field"><legend className="field-label">指标</legend><div className="gateway-node-metric-checks">{[...metrics].sort((a,b) => (selected.includes(a.key) ? selected.indexOf(a.key) : metrics.length) - (selected.includes(b.key) ? selected.indexOf(b.key) : metrics.length)).map((metric) => <label key={metric.key} draggable={selected.includes(metric.key)} onDragStart={() => { draggedMetric.current = metric.key; }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const from = selected.indexOf(draggedMetric.current ?? ""); const to = selected.indexOf(metric.key); if (from < 0 || to < 0 || from === to) return; const next = [...selected]; next.splice(to, 0, next.splice(from, 1)[0]); setSelected(next); draggedMetric.current = null; }} className={selected.includes(metric.key) ? "selected" : ""}><input type="checkbox" checked={selected.includes(metric.key)} onChange={(event) => setSelected((current) => event.target.checked ? [...current, metric.key] : current.filter((key) => key !== metric.key))}/><span><strong>{metric.name}</strong><small>{metric.unit}</small></span></label>)}</div></fieldset>}
