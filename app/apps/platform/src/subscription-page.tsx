...
export function SubscriptionPage() {
  ...
  if (!currentId) return <Panel><StateView type="empty" title="请选择组织" description="订阅权益归属于组织，请先选择一个组织。" /></Panel>;
  ...
  return <>
    <PageHeader eyebrow="组织 / 订阅" title="订阅与用量" description={`${current?.name ?? "当前组织"}的套餐权益、下载用量与开放 API。`} actions={<Button variant="secondary" onClick={() => navigate("/workspaces")}><ArrowLeft size={14} />返回组织</Button>} />
    <div className="subscription-page">
      ...
      <section className={`subscription-hero ${professional ? "is-pro" : ""}`}>
        ...
        <div className="subscription-plan-copy"><span>当前套餐</span><h2>{professional ? "专业版" : "基础版"}</h2><p>{professional ? `服务有效至 ${date(value.professional_expires_at)}，组织全部成员共享权益。` : "永久免费，设备接入、采集和基础查看不受设备数量限制。"}</p></div>
        <div className="subscription-plan-side"><Badge tone={professional ? "success" : "neutral"}>{professional ? "服务中" : "永久免费"}</Badge><strong>{professional ? price : "¥0"}</strong><span>{professional ? "/ 组织 / 年" : "无需续费"}</span></div>
      </section>
      ...
      <section className="pricing-section">
        <div className="pricing-heading"><span>套餐权益</span><h2>基础能力永久免费，按需开通专业数据服务</h2><p>套餐归属于组织，全部成员共享权益，不按设备数量收费。</p></div>
        <div className="pricing-grid">
          ...
          <article className={`pricing-card pricing-card-pro ${professional ? "is-current" : ""}`}>
            ...
            <div className="pricing-price"><strong>{price}</strong><span>/ 组织 / 年</span></div>
            <div className="pricing-divider" />
            <ul>
              ...
              <li><Check size={16} /><span>开放 API 与组织 API Key</span></li>
