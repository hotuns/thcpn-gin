...
export function GatewayOverview({ device, workspaceId }: { device: Device; workspaceId: string }) {
  ...
  return <div className="gateway-overview">
    ...
    {nodes.error ? <StateView type="error" title="节点读取失败" description={formatApiError(nodes.error).message}/> : nodes.isLoading ? <StateView type="loading" title="正在读取节点" description="正在汇总节点与指标。"/> : <>
      ...
      {rows.length ? <div className="gateway-node-table"><div className="gateway-node-table-head"><span>节点</span><span>数据状态</span><span>指标</span><span>最近数据</span><span/></div>{rows.map(({node,latest,loading,error,complete}) => <button key={node.key} type="button" className="gateway-node-row" onClick={() => open(node.key)}><strong>{node.name}</strong><Badge tone={error ? "warning" : latest ? "success" : "neutral"}>{loading ? "读取中" : error ? "读取失败" : !node.streams.length ? "暂无指标" : latest ? "有数据" : "无近期数据"}</Badge><span>{node.streams.length} 个{!complete && " · 未完整读取"}</span><time>{error ? typeof error === "string" ? error : formatApiError(error).message : latest ? new Date(latest).toLocaleString() : "—"}</time><MoveRight size={15}/></button>)}</div> : <StateView type="empty" title="暂无节点" description="当前组网站尚未配置节点。"/>}
      </Panel>
    </>}
  </div>;
}
