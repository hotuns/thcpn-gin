...
import { Badge, Button, Checkbox, CloseButton, Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, Input, PageHeader, Panel, Select, SelectContent, SelectItem, SelectTrigger, SelectValue, StateView } from "./platform-ui";
...
function WallboardEditor({ workspaceId, template, wallboard, onClose, onSaved }: { workspaceId: string; template: WallboardTemplate | null; wallboard: Wallboard | null; onClose: () => void; onSaved: (item: Wallboard) => void }) {
  ...
  const selectedDeviceIds = template?.scene === "device" && deviceId ? [deviceId] : [];
  ...
  return <Dialog open onOpenChange={(open) => !open && !busy && onClose()}><DialogContent className="shadcn-wallboard-editor"><DialogHeader><DialogTitle>{wallboard ? "编辑大屏" : "配置模板"} · {template.name}</DialogTitle><DialogDescription>选择展示范围、趋势周期和需要出现在大屏中的数据流。</DialogDescription></DialogHeader><div className="wallboard-editor-grid shadcn-dialog-form"><label><span>大屏名称</span><Input value={name} onChange={(e) => setName(e.target.value)}/></label>{template.scene === "device" && <label><span>目标设备</span><DeviceCombobox devices={devices.data?.items ?? []} value={deviceId} onChange={(id) => {setDeviceId(id);setTelemetryIds([]);setImageIds([])}} ariaLabel="搜索大屏目标设备"/></label>}<label><span>趋势时间范围</span><Select value={trendHours} onValueChange={setTrendHours}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="24">最近 24 小时</SelectItem><SelectItem value="72">最近 3 天</SelectItem><SelectItem value="168">最近 7 天</SelectItem><SelectItem value="720">最近 30 天</SelectItem></SelectContent></Select></label></div>{template.scene === "organization" && <MultiDeviceSelector devices={devices.data?.items ?? []} selected={deviceIds} onAdd={(id)=>{if(!deviceIds.includes(id)&&deviceIds.length<50)setDeviceIds([...deviceIds,id])}} onRemove={(id)=>setDeviceIds(deviceIds.filter((value)=>value!==id))}/>} {template.scene === "device" && <><StreamChoices title="实时与趋势指标" items={telemetry} selected={telemetryIds} toggle={(id) => toggle(id,telemetryIds,setTelemetryIds)}/><StreamChoices title="图片数据流" items={images} selected={imageIds} toggle={(id) => toggle(id,imageIds,setImageIds)}/></>}{error && <div className="command-note">{error}</div>}<DialogFooter><Button variant="secondary" onClick={onClose}>取消</Button><Button disabled={busy || !name || (template.scene === "device" && !deviceId)} onClick={() => void save()}>{busy ? "正在保存…" : "保存并预览"}</Button></DialogFooter></DialogContent></Dialog>;
}
function MultiDeviceSelector({devices,selected,onAdd,onRemove}:{devices:Awaited<ReturnType<typeof api.devices.list>>["items"];selected:string[];onAdd:(id:string)=>void;onRemove:(id:string)=>void}) { const selectedDevices=devices.filter((item)=>selected.includes(item.id));return <section className="wallboard-streams wallboard-device-selector"><h3>展示设备（不选则展示组织全部，最多 50 台）</h3><DeviceCombobox devices={devices} value="" onChange={onAdd} ariaLabel="搜索并添加展示设备" placeholder="搜索并添加设备"/>{selectedDevices.length?<div>{selectedDevices.map((item)=><label key={item.id}><Checkbox checked onCheckedChange={()=>onRemove(item.id)}/><span>{item.name}</span></label>)}</div>:<p>当前将展示组织全部设备。</p>}</section> }
function StreamChoices({ title, items, selected, toggle }: { title: string; items: Array<{ id: string; name: string; unit?: string }>; selected: string[]; toggle: (id: string) => void }) { return <section className="wallboard-streams"><h3>{title}</h3>{!items.length ? <p>选择目标资源后显示可用数据流。</p> : <div>{items.map((item) => <label key={item.id}><Checkbox checked={selected.includes(item.id)} onCheckedChange={() => toggle(item.id)}/><span>{item.name}{item.unit ? ` · ${item.unit}` : ""}</span></label>)}</div>}</section>; }
