...
function FleetWallboard({ template, snapshot }: { template: WallboardTemplate; snapshot: WallboardSnapshot }) {
  ...
  return <div className="wallboard-canvas wallboard-fleet-layout"><WallboardHeader template={template} snapshot={snapshot}/><div className="wallboard-fleet-dashboard">
    <aside className="wallboard-fleet-rail"><ScreenPanel title="全域运行概览"><div className="wallboard-fleet-kpis"><Kpi label="设备总数" value={snapshot.devices.length}/><Kpi label="在线率" value={`${onlineRate}%`}/><Kpi label="覆盖站点" value={sites.size}/><Kpi label="待关注" value={warning}/></div><div className="wallboard-fleet-health"><span><i className="online"/>在线 {online}</span><span><i/>离线 {offline}</span><span><i className="alarm"/>告警 {alarms}</span><span><i className="unavailable"/>不可用 {unavailable}</span></div></ScreenPanel><ScreenPanel title={`待关注设备 · ${warning}`}><div className="wallboard-attention-list">{snapshot.devices.filter(needsAttention).slice(0, 8).map((device) => <button type="button" className={device.id === selected?.id ? "selected" : ""} key={device.id} onClick={() => setSelectedId(device.id)}><span className={`wallboard-status ${statusClass(device)}`}/><strong>{device.name}</strong><small>{attentionReason(device)}</small></button>)}{!warning && <EmptyLine text="当前无异常设备"/>}</div></ScreenPanel></aside>
    <main className="wallboard-fleet-center"><section className="wallboard-map-panel"><div className="wallboard-map-summary"><span>设备空间分布</span><small>{snapshot.devices.length - unlocated} 个有效点位 · {unlocated} 台未定位</small></div><WallboardCesiumMap devices={snapshot.devices} mode="multi" selectedDeviceId={selected?.id} onSelectDevice={setSelectedId}/>{unlocated > 0 && <LocationNotice count={unlocated}/>}</section><TrendPanel metrics={snapshot.metrics} devices={snapshot.devices}/></main>
    <aside className="wallboard-fleet-media"><ScreenPanel title="当前设备">{selected ? <FleetDeviceFocus device={selected}/> : <EmptyLine text="暂无设备"/>}</ScreenPanel><ImagePanel items={latestImageItems(snapshot)} limit={5}/></aside>
  </div></div>;
}
