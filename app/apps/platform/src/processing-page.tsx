...
export function ProcessingPage() {
  ...
  const plans = useQuery({ queryKey: ["processing", "plans"], queryFn: api.processing.plans });
  ...
  return <>
    ...
    <div className="processing-summary" data-onboarding="processing-summary">
      ...
      <div><strong>{plans.data?.items.length ?? "—"}</strong><span>可用方案</span></div>
...
export function ProcessingTaskDetailPage() {
  ...
  return <>
    ...
    <div className="processing-detail-summary"><div><span>处理方案</span><strong>{item.plan_name || "历史直接配置任务"}</strong></div><div><span>任务状态</span><Badge tone={statusTone(item.status) as any}>{statusLabel[item.status]}</Badge></div><div><span>处理起点</span><strong>{displayTime(item.start_at)}</strong></div><div><span>执行次数</span><strong>{executions.data?.items.length ?? "—"}</strong></div></div>
...
function ProcessingResultsView({ executions, outputNames }: { executions: ProcessingExecution[]; outputNames: Map<string, string> }) {
  ...
  return <div className="processing-outcomes">
    <div className="processing-outcome-toolbar"><label><span>成果指标</span><SelectInput value={selected?.code ?? ""} onChange={(event) => setSelectedCode(event.target.value)}>{metrics.map((metric) => <option key={metric.code} value={metric.code}>{metric.name}{metric.unit ? ` · ${metric.unit}` : ""}</option>)}</SelectInput></label><div className="processing-outcome-summary"><div><span>最新值</span><strong>{latest?.value.toLocaleString(document.documentElement.lang || "zh-CN")}<small>{selected?.unit}</small></strong></div><div><span>成果数量</span><strong>{selectedPoints.length}<small>条</small></strong></div><div><span>时间范围</span><strong>{selectedPoints.length > 1 ? `${displayOutcomeDate(selectedPoints[0].observedAt)} - ${displayOutcomeDate(latest?.observedAt)}` : displayOutcomeDate(latest?.observedAt)}</strong></div></div></div>
...
function CreateProcessingTask({ workspaceId, plans, onClose, onCreated }: { workspaceId: string; plans: ProcessingPlan[]; onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  const [roi,setRoi]=useState<CalibrationBoard>();
  ...
  const selected = useMemo(() => plans.find((item) => item.id === planId), [plans, planId]);
  const slots = selected?.processor_manifest.inputs ?? [];
  useEffect(() => {
    const previous = document.body.style.overflow;
    document.body.style.overflow = "hidden";
    return () => { document.body.style.overflow = previous; };
  }, []);
  useEffect(() => { if (targetType === "device" && targetId) setSourceDevices(Object.fromEntries(slots.map((slot) => [slot.code, targetId]))); }, [targetId, targetType, planId]);
  const submit = async (event: FormEvent) => {
    event.preventDefault(); if (!selected || !targetId) return;
    setBusy(true); setError("");
    try {
      await api.processing.create(workspaceId, { name, description, plan_id: selected.id, target_type: targetType, target_id: targetId, interaction: roi ? {roi} : {}, start_at: startMode === "now" ? new Date().toISOString() : new Date(startAt).toISOString(), inputs: slots.map((slot) => ({ slot_code: slot.code, source_type: "data_stream", source_id: sourceStreams[slot.code], config: sourceConfigs[slot.code] ?? {} })) });
      await onCreated();
  ...
  return <div className="processing-dialog"><form className="processing-editor" role="dialog" aria-modal="true" aria-label="创建处理任务" onSubmit={submit}>
    <div className="panel-header"><div><h2 className="panel-title">创建处理任务</h2><div className="panel-kicker">步骤 {step} / 4 · 基于运维发布的处理方案</div></div><Button type="button" variant="secondary" onClick={onClose}><X size={14}/>关闭</Button></div>
    <div className="processing-stepper">{["选择方案","绑定输入","图像配置","确认创建"].map((label,index)=><span key={label} className={step===index+1?"is-active":step>index+1?"is-done":""}>{index+1}. {label}</span>)}</div>
    <div className="processing-form"><div className="processing-form-main">
    {step===1&&<div className="processing-plan-options">{plans.map((plan)=><button type="button" key={plan.id} className={planId===plan.id?"is-active":""} onClick={()=>{setPlanId(plan.id);setTargetId("");setSourceStreams({});setSourceConfigs({});setRoi(undefined)}}><strong>{plan.name}</strong><span>{plan.description}</span><small>{plan.processor_code}@{plan.processor_version} · 输出 {(plan.processor_manifest.outputs??[]).map(item=>item.name).join("、")}</small></button>)}</div>}
    {step===2&&<><div className="form-grid two"><label className="field"><span className="field-label">结果目标</span><SelectInput value={targetType} onChange={(e) => { setTargetType(e.target.value as any); setTargetId(""); }}><option value="device">设备</option><option value="site">站点</option></SelectInput></label><label className="field"><span className="field-label">目标资源</span>{targetType==="device"?<DeviceCombobox devices={devices.data?.items??[]} value={targetId} onChange={setTargetId} ariaLabel="搜索目标设备"/>:<SelectInput required value={targetId} onChange={(e) => setTargetId(e.target.value)}><option value="">请选择</option>{sites.data?.items.map((item: any) => <option key={item.id} value={item.id}>{item.name}</option>)}</SelectInput>}</label></div><div className="processing-inputs"><div className="section-label">输入绑定</div>{slots.map((slot) => <InputBinding key={slot.code} slot={slot} devices={devices.data?.items ?? []} deviceId={sourceDevices[slot.code] ?? ""} streamId={sourceStreams[slot.code] ?? ""} config={sourceConfigs[slot.code] ?? {}} showDevice={targetType === "site"} showCalibration={false} onDevice={(value) => setSourceDevices((current) => ({...current, [slot.code]: value}))} onStream={(value) => { setSourceStreams((current) => ({...current, [slot.code]: value})); setSourceConfigs((current) => ({...current, [slot.code]: {}})); }} onConfig={(value) => setSourceConfigs((current) => ({...current, [slot.code]: value}))}/>)}</div></>}
    {step===3&&<div className="processing-image-workbench">{selected?.processor_manifest.ui?.analysis_roi?.required&&sourceStreams[slots[0]?.code]&&<CalibrationBoardPicker streamId={sourceStreams[slots[0].code]} label="共享分析 ROI" value={roi} onChange={setRoi}/>} {slots.filter(slot=>slot.ui?.calibration_board).map(slot=><CalibrationBoardPicker key={slot.code} streamId={sourceStreams[slot.code]} label={slot.ui?.calibration_board?.label??"标定板"} value={sourceConfigs[slot.code]?.calibration_board as CalibrationBoard|undefined} onChange={(value)=>setSourceConfigs(current=>({...current,[slot.code]:{...(current[slot.code]??{}),calibration_board:value}}))}/>) } {!selected?.processor_manifest.ui?.analysis_roi?.required&&!slots.some(slot=>slot.ui?.calibration_board)&&<StateView type="empty" title="无需图像区域配置" description="该方案会直接处理所选输入。"/>}</div>}
    {step===4&&<><div className="form-grid two"><label className="field"><span className="field-label">任务名称</span><Input required value={name} onChange={(e) => setName(e.target.value)}/></label><label className="field"><span className="field-label">处理起点</span><SelectInput value={startMode} onChange={(e) => setStartMode(e.target.value)}><option value="now">从现在开始</option><option value="history">从指定时间开始</option></SelectInput></label></div>{startMode === "history" && <label className="field"><span className="field-label">开始时间</span><Input required type="datetime-local" value={startAt} onChange={(e) => setStartAt(e.target.value)}/></label>}<label className="field"><span className="field-label">说明</span><Input value={description} onChange={(e) => setDescription(e.target.value)}/></label><div className="processor-contract"><Activity size={15}/><div><strong>{selected?.name}</strong><span>{selected?.description}</span><small>{targetType==="device"?"设备":"站点"} · {slots.length} 个输入 · 输出 {(selected?.processor_manifest.outputs??[]).map(item=>item.name).join("、")}</small></div></div></>}
    </div>
    <aside className="processing-form-side">
    {selected && <div className="processor-contract"><Activity size={15}/><div><strong>{selected.name}</strong><span>{selected.description}</span><small>方案 v{selected.published_version} · {selected.processor_manifest.category==="timeseries"?"数值处理":"图像处理"}</small></div></div>}
    {error && <div className="form-error">{error}</div>}<div className="form-actions">{step>1&&<Button type="button" variant="secondary" onClick={()=>setStep(step-1)}>上一步</Button>}{step<4?<Button type="button" disabled={(step===1&&!selected)||(step===2&&(!targetId||slots.some(slot=>!sourceStreams[slot.code])))||(step===3&&((selected?.processor_manifest.ui?.analysis_roi?.required&&!roi)||slots.some(slot=>slot.ui?.calibration_board?.required&&!sourceConfigs[slot.code]?.calibration_board)))} onClick={()=>setStep(step+1)}>下一步</Button>:<Button type="submit" disabled={busy || !name}>{busy ? "创建中…" : "创建任务"}</Button>}</div></aside></div>
  </form></div>;
}
...
function InputBinding({ slot, devices, deviceId, streamId, config, showDevice = true, showCalibration = true, onDevice, onStream, onConfig }: { slot: InputSlot; devices:Device[]; deviceId:string;streamId:string;config:JsonRecord;showDevice?:boolean;showCalibration?:boolean;onDevice:(value:string)=>void;onStream:(value:string)=>void;onConfig:(value:JsonRecord)=>void }) {
  const streams = useQuery({ queryKey: ["data-streams", deviceId], queryFn: () => api.dataStreams.list(deviceId), enabled: Boolean(deviceId) });
  return <div className="processing-input-binding"><div className={`processing-input-row ${showDevice ? "" : "without-device"}`}><div><strong>{slot.name}</strong><span className="mono">{slot.code} · {slot.kind}</span></div>{showDevice&&<DeviceCombobox devices={devices} value={deviceId} onChange={(value)=>{onDevice(value);onStream("");}} ariaLabel={`搜索${slot.name}来源设备`}/>}<SelectInput required value={streamId} onChange={(e) => onStream(e.target.value)}><option value="">选择数据流</option>{streams.data?.items.filter((item) => item.type === "image").map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</SelectInput></div>{showCalibration && slot.ui?.calibration_board && streamId ? <CalibrationBoardPicker streamId={streamId} label={slot.ui.calibration_board.label ?? "标定板"} value={config.calibration_board as CalibrationBoard | undefined} onChange={(value) => onConfig({...config, calibration_board: value})} /> : null}</div>;
}
...
function CalibrationBoardPicker({ streamId, label, value, onChange }: { streamId: string; label: string; value?: CalibrationBoard; onChange: (value: CalibrationBoard) => void }) {
  ...
  const media = useQuery({ queryKey: ["processing", "latest-image", streamId], queryFn: () => api.media.dataStream(streamId, { start_time: startTime.toISOString(), end_time: endTime.toISOString(), page: 1, page_size: 1 }), enabled: Boolean(streamId) });
  const image = media.data?.items[0] as MediaItem | undefined;
  if (media.isLoading) return <div className="processing-calibration-state">正在加载最新图片…</div>;
  if (media.error) return <div className="processing-calibration-state is-error">图片加载失败：{formatApiError(media.error).message}</div>;
  if (!image) return <div className="processing-calibration-state is-error">该数据流最近 30 天没有可用于框选的图片</div>;
