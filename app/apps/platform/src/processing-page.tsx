...
import { api, formatApiError, type JsonRecord, type MediaItem, type ProcessingExecution, type ProcessingPlan, type ProcessingProcessor, type ProcessingResult, type ProcessingTask } from "@thcpn/api";
...
function CreateProcessingTask({ workspaceId, plans, onClose, onCreated }: { workspaceId: string; plans: ProcessingPlan[]; onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  return <div className="processing-dialog"><form className="processing-editor" role="dialog" aria-modal="true" aria-label="创建处理任务" onSubmit={submit}>
    ...
    {step===2&&<><div className="form-grid two"><label className="field"><span className="field-label">结果目标</span><select value={targetType} onChange={(e) => { setTargetType(e.target.value as any); setTargetId(""); }}><option value="device">设备</option><option value="site">站点</option></select></label><label className="field"><span className="field-label">目标资源</span><select required value={targetId} onChange={(e) => setTargetId(e.target.value)}><option value="">请选择</option>{(targetType === "device" ? devices.data?.items : sites.data?.items)?.map((item: any) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div><div className="processing-inputs"><div className="section-label">输入绑定</div>{slots.map((slot) => <InputBinding key={slot.code} slot={slot} devices={devices.data?.items ?? []} deviceId={sourceDevices[slot.code] ?? ""} streamId={sourceStreams[slot.code] ?? ""} config={sourceConfigs[slot.code] ?? {}} showDevice={targetType === "site"} showCalibration={false} onDevice={(value) => setSourceDevices((current) => ({...current, [slot.code]: value}))} onStream={(value) => { setSourceStreams((current) => ({...current, [slot.code]: value})); setSourceConfigs((current) => ({...current, [slot.code]: {}})); }} onConfig={(value) => setSourceConfigs((current) => ({...current, [slot.code]: value}))}/>)}</div></>}
    ...
    {step===4&&<><div className="form-grid two"><label className="field"><span className="field-label">任务名称</span><input required value={name} onChange={(e) => setName(e.target.value)}/></label><label className="field"><span className="field-label">处理起点</span><select value={startMode} onChange={(e) => setStartMode(e.target.value)}><option value="now">从现在开始</option><option value="history">从指定时间开始</option></select></label></div>{startMode === "history" && <label className="field"><span className="field-label">开始时间</span><input required type="datetime-local" value={startAt} onChange={(e) => setStartAt(e.target.value)}/></label>}<label className="field"><span className="field-label">说明</span><input value={description} onChange={(e) => setDescription(e.target.value)}/></label><div className="processor-contract"><Activity size={15}/><div><strong>{selected?.name}</strong><span>{selected?.description}</span><small>{targetType==="device"?"设备":"站点"} · {slots.length} 个输入 · 输出 {(selected?.processor_manifest.outputs??[]).map(item=>item.name).join("、")}</small></div></div></>}
    </div>
...
function InputBinding({ slot, devices, deviceId, streamId, config, showDevice = true, showCalibration = true, onDevice, onStream, onConfig }: { slot: InputSlot; devices: any[]; deviceId:string;streamId:string;config:JsonRecord;showDevice?:boolean;showCalibration?:boolean;onDevice:(value:string)=>void;onStream:(value:string)=>void;onConfig:(value:JsonRecord)=>void }) {
  const streams = useQuery({ queryKey: ["data-streams", deviceId], queryFn: () => api.dataStreams.list(deviceId), enabled: Boolean(deviceId) });
  return <div className="processing-input-binding"><div className={`processing-input-row ${showDevice ? "" : "without-device"}`}><div><strong>{slot.name}</strong><span className="mono">{slot.code} · {slot.kind}</span></div>{showDevice&&<select required value={deviceId} onChange={(e) => {onDevice(e.target.value);onStream("");}}><option value="">选择设备</option>{devices.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>}<select required value={streamId} onChange={(e) => onStream(e.target.value)}><option value="">选择数据流</option>{streams.data?.items.filter((item) => item.type === "image").map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>{showCalibration && slot.ui?.calibration_board && streamId ? <CalibrationBoardPicker streamId={streamId} label={slot.ui.calibration_board.label ?? "标定板"} value={config.calibration_board as CalibrationBoard | undefined} onChange={(value) => onConfig({...config, calibration_board: value})} /> : null}</div>;
}
