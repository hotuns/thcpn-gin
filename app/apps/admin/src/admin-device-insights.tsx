...
const kinds = [
  ["ecosystem", "设备类型"], ["observation_object", "观测对象"], ["purpose", "观测用途"], ["management", "管理方式"], ["deployment", "部署环境"],
] as const;
...
export function AdminDeviceInsightsPage() {
  ...
  return <>
    <PageHeader eyebrow="Assets / geography" title="设备地图与分类" description="按设备类型、观测对象和用途统计设备资产，并查看精确空间分布。" actions={<Button icon={<RefreshCw size={14} />} onClick={() => void Promise.all([mapQuery.refetch(), catalogQuery.refetch()])}>刷新</Button>} />
    ...
    <Panel className="device-insight-panel">
      <Tabs items={[
        { key:"map", label:<span><MapPin size={14}/>地图与统计</span>, children:<><div className="device-insight-toolbar"><Select allowClear placeholder="全部设备类型" value={ecosystem} options={options("ecosystem")} onChange={setEcosystem}/><Select allowClear placeholder="全部观测用途" value={purpose} options={options("purpose")} onChange={setPurpose}/><label><Switch checked={includeChildren} onChange={setIncludeChildren}/> 显示子节点</label>{selected.length ? <Button type="primary" onClick={()=>setBulkOpen(true)}>批量设置 {selected.length} 台</Button>:null}</div>{mapQuery.isLoading?<StateView type="loading" title="正在加载设备地图" description="正在计算有效位置和分类。"/>:mapQuery.error?<StateView type="error" title="设备地图加载失败" description={formatApiError(mapQuery.error).message}/>:<><DeviceMap height={480} styleUrl={token ? undefined : import.meta.env.VITE_MAP_STYLE_URL} mapStyle={mapStyle} points={rows.map(mapPoint)} onSelect={(id)=>window.location.assign(`/admin/devices/${id}`)}/><Table rowKey="device_id" size="small" rowSelection={{selectedRowKeys:selected,onChange:setSelected}} dataSource={rows} pagination={{pageSize:20,showSizeChanger:true}} columns={[{title:"设备",render:(_,item)=><div><strong>{item.name}</strong><div className="cell-sub mono">{item.serial_no}</div></div>},{title:"设备类型",width:140,render:(_,item)=>{const term=item.environment.ecosystem;const src=iconifyIconUrl(term?.icon);return <Space>{src?<img src={src} width={18} height={18} alt=""/>:null}{name(term)}</Space>}},{title:"用途",render:(_,item)=><Space size={2} wrap>{item.environment.purposes.map((term)=><Tag key={term.id}>{term.name_zh}</Tag>)}</Space>},{title:"位置",width:110,render:(_,item)=>item.latitude!==undefined?<Tag color="green">{locationLabel(item.location_source)}</Tag>:<Tag>未定位</Tag>},{title:"状态",width:90,dataIndex:"status"}]}/></>}</> },
...
function BulkEnvironmentModal({open,deviceIds,terms,onClose,onSaved}:{open:boolean;deviceIds:string[];terms:DeviceTaxonomyTerm[];onClose:()=>void;onSaved:()=>Promise<void>}) {
  const mapQuery=useQuery({queryKey:["admin","device-map",false],queryFn:()=>api.admin.deviceMap(false),enabled:open});
  const tagOptions=Array.from(new Set((mapQuery.data?.items??[]).flatMap((item)=>item.environment.research_tags))).sort().map((value)=>({value,label:value}));
  ...
  const options=(kind:string)=>terms.filter((term)=>term.kind===kind&&term.status==="active").map((term)=>({value:term.id,label:term.name_zh}));
  return <Modal title={`批量设置设备属性 · ${deviceIds.length} 台`} open={open} onCancel={onClose} okText="应用设置" confirmLoading={mutation.isPending} onOk={()=>void form.validateFields().then((values)=>mutation.mutate(values))} destroyOnHidden><Form form={form} layout="vertical"><Form.Item name="ecosystem_term_id" label="设备类型"><Select allowClear options={options("ecosystem")}/></Form.Item><Form.Item name="observation_object_ids" label="观测对象"><Select mode="multiple" options={options("observation_object")}/></Form.Item><Form.Item name="purpose_ids" label="观测用途"><Select mode="multiple" options={options("purpose")}/></Form.Item><div className="drawer-grid"><Form.Item name="management_term_id" label="管理方式"><Select allowClear options={options("management")}/></Form.Item><Form.Item name="deployment_term_id" label="部署环境"><Select allowClear options={options("deployment")}/></Form.Item></div><Form.Item name="commissioned_year" label="投运年份"><InputNumber min={1900} max={2200} style={{width:"100%"}}/></Form.Item><Form.Item name="research_tags" label="研究标签"><Select mode="tags" tokenSeparators={[","]} options={tagOptions} placeholder="选择已有标签或输入新标签"/></Form.Item></Form></Modal>;
}
...
function mapPoint(item:DeviceMapItem){return{device_id:item.device_id,name:item.name,device_type:item.device_type,status:item.status,latitude:item.latitude,longitude:item.longitude,child_count:item.child_count,ecosystem:item.environment.ecosystem?.name_zh,category_icon:item.environment.ecosystem?.icon,purposes:item.environment.purposes.map((term)=>term.name_zh)}}
