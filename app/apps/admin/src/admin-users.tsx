...
function DemoShowcasePanel(){const [keyword,setKeyword]=useState("");const client=useQueryClient();const {message}=AntApp.useApp();const query=useQuery({queryKey:["admin","demo-showcase","devices",keyword],queryFn:()=>api.admin.demoShowcase.devices(keyword)});const change=async(row)=>{try{if(row.selected)await api.admin.demoShowcase.removeDevice(row.id);else await api.admin.demoShowcase.addDevice(row.id);await client.invalidateQueries({queryKey:["admin","demo-showcase","devices"]});void message.success(row.selected?"已移出演示清单":"已加入演示清单")}catch(error){void message.error(formatApiError(error).message)}};return <div><Alert type="info" showIcon message="精选设备保留原组织归属，演示账号将在演示空间中跨组织查看和操作。转移、解绑、认领和删除始终禁止。"/><Input.Search style={{margin:"16px 0",maxWidth:480}} value={keyword} onChange={(event)=>setKeyword(event.target.value)} placeholder="搜索设备名称或 SN" allowClear/><Table rowKey="id" loading={query.isLoading} dataSource={query.data?.items??[]} pagination={{pageSize:20}} columns={[{title:"设备",render:(_,row)=><div><strong>{row.name}</strong><div className="cell-sub mono">{row.serial_no}</div></div>},{title:"形态",dataIndex:"device_type",width:100},{title:"原组织",render:(_,row)=><div>{text(row.workspace_name,"未分配")}<div className="cell-sub">{[row.project_name,row.site_name].filter(Boolean).join(" / ")||"未设置项目/样地"}</div></div>},{title:"状态",dataIndex:"status",width:90},{title:"演示清单",width:130,render:(_,row)=>row.selected?<Button danger icon={<Trash2 size={14}/>} onClick={()=>void change(row)}>移出</Button>:<Button type="primary" onClick={()=>void change(row)}>加入</Button>}]}/></div>}
