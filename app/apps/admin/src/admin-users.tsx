...
import { ArrowLeft, Copy, KeyRound, LockKeyhole, Plus, RefreshCw, ShieldCheck, UserRound, UserX } from "lucide-react";
...
export function AdminUsersPage() {
  ...
  const columns = [
    ...
    { title: "Workspace", dataIndex: "workspace_count", width: 110, render: (value) => `${value ?? 0} 个` },
  ...
  return <>
    ...
    <Panel className="admin-user-list-panel">
      ...
      {query.isError ? <ErrorState error={query.error} /> : <Table rowKey="id" loading={query.isLoading} columns={columns} dataSource={items} scroll={{ x: 1080 }} pagination={{ current: filters.page, pageSize: 20, total: query.data?.total ?? 0, showSizeChanger: false, onChange: (page) => apply("page", String(page)) }} />}
    </Panel>
    <Modal title="创建平台用户" open={createOpen} onCancel={() => setCreateOpen(false)} onOk={() => void create()} okText="创建用户" destroyOnClose><Alert type="info" showIcon message="系统会生成一次性临时密码，用户首次登录必须修改。" style={{ marginBottom: 16 }} /><Form form={form} layout="vertical"><Form.Item name="name" label="姓名" rules={[{ required: true, message: "请输入姓名" }]}><Input /></Form.Item><Form.Item name="email" label="邮箱"><Input type="email" /></Form.Item><Form.Item name="phone" label="手机号"><Input /></Form.Item><Form.Item name="reason" label="创建原因" rules={[{ required: true, min: 5, message: "请填写至少 5 个字符的原因" }]}><Input.TextArea rows={3} /></Form.Item></Form></Modal>
