...
func registerAPIV1(router *gin.Engine, deps Dependencies, cfg config.Config) error {
	...
	admin.POST("/workspaces/:workspace_id/interventions", workspaceHandler.AdminStartIntervention)
	admin.DELETE("/workspaces/:workspace_id/interventions/:intervention_id", workspaceHandler.AdminEndIntervention)
	admin.PATCH("/workspaces/:workspace_id/status", workspaceHandler.RequireWorkspaceIntervention(), workspaceHandler.AdminUpdateStatus)
	admin.POST("/workspaces/:workspace_id/transfer-owner", workspaceHandler.RequireWorkspaceIntervention(), workspaceHandler.AdminTransferOwner)
	...
	admin.POST("/workspaces/:workspace_id/members", workspaceHandler.RequireWorkspaceIntervention(), memberHandler.Add)
	admin.PATCH("/workspaces/:workspace_id/members/:member_id", workspaceHandler.RequireWorkspaceIntervention(), memberHandler.UpdateRole)
	admin.DELETE("/workspaces/:workspace_id/members/:member_id", workspaceHandler.RequireWorkspaceIntervention(), memberHandler.Remove)
	admin.GET("/access-grants", accessGrantHandler.ListGrants)
	admin.POST("/access-grants", workspaceHandler.TenantManagedOperation)
	admin.DELETE("/access-grants/:access_grant_id", workspaceHandler.RequireResourceIntervention("access_grant", "access_grant_id"), accessGrantHandler.RevokeGrant)
	admin.GET("/invitations", accessGrantHandler.ListInvitations)
	admin.POST("/invitations", workspaceHandler.TenantManagedOperation)
	admin.DELETE("/invitations/:invitation_id", workspaceHandler.RequireResourceIntervention("invitation", "invitation_id"), accessGrantHandler.RevokeInvitation)
	admin.GET("/audit-logs", auditHandler.List)
	admin.POST("/projects", workspaceHandler.TenantManagedOperation)
	admin.PATCH("/projects/:project_id", workspaceHandler.RequireResourceIntervention("project", "project_id"), projectHandler.Update)
	admin.POST("/sites", workspaceHandler.TenantManagedOperation)
	admin.PATCH("/sites/:site_id", workspaceHandler.RequireResourceIntervention("site", "site_id"), siteHandler.Update)
