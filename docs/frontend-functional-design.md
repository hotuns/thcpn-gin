...
本文档定义 THCPN Web 当前已实现的功能基线，供开发、联调、测试和产品验收使用。
...
- 关键业务流程。
- 页面状态、交互规则和 API 边界。
- 前端页面、路由和领域 API 的当前职责。

本文档描述业务能力和交互契约，不展开具体 CSS、组件文件或视觉稿。当前前端按 React 页面、领域 API client、TanStack Query 查询缓存和按路由懒加载组织；页面使用的 Ant Design、ECharts 和自定义 UI 组件属于现有实现的一部分。

本文档不是项目进度表，不记录负责人、迭代排期或临时验收结果。功能契约发生变化时直接修改对应章节，并由 Git 历史保留变更过程。
...
| `/devices/:deviceId?tab=data` | 数据 / 设备数据 | 单个设备的遥测、图片和相机实时视频查询，以及保存为 Dataset |
...
设置中的基础资料、访问控制、审计和账号安全通过 URL query 保持可深链接状态。旧入口 `/device-data` 保留为兼容跳转，会根据设备参数打开对应设备详情的数据页。
...
- 按全部、网关、相机、标准站分类并显示数量；节点在所属网关下展开查看，不单独作为顶层设备。
...
- 原始数据流由系统同步生成，计算数据流由设备公式配置生成；Workspace 用户按权限查看可用数据流。
- 用户先选择设备，再查看该设备的数据流。
...
- 用户可以为设备配置一个计算数据流和公式。
- 公式可以引用同一设备的原始遥测数据流和数字型设备元数据。
- 计算结果查询时生成，不在平台业务库持久化；历史范围重新查询时按当前公式和元数据计算。
- 公式和元数据修改只保留当前值，修改操作写入审计日志。
...
- 指标支持按名称和编码搜索；勾选直接加入草稿，取消勾选直接移除，无需再次点击添加。全选与清空只作用于当前搜索结果，不影响其他设备的已选来源。
- 创建表单按基本信息、数据来源分区展示，提交摘要显示名称、来源数量、时间范围和项目；无效时间范围不得提交。
- 至少选择一个有效来源。
...
- `/exports` 仅展示任务列表；新建使用独立页面 `/exports/new`。旧的带 `system` 查询参数入口保留参数跳转至新建页面。取消返回列表，创建成功后返回列表查看进度。
- 创建时按设备体系选择设备或节点，再指定时间和导出内容；选择区与提交摘要分栏，小屏改为上下布局。
- 时间范围支持最近 1、3、7 天快捷选择；空目标、无效时间或碳汇未选择数据内容时禁用提交。
...
- 同步结果先进入系统设备资产，不在同步阶段选择 Workspace、Project 或 Site。
- 展示同步后的 Device、配置快照、DataStream、Binding 和禁用项摘要。

网关同步：

- 选择 DataSource 和外部网关设备。
- 输入网关外部 ID 后同步网关、节点、关系、DataStream 和 Binding。
- 节点不单独分配工作区；分配网关后，节点权限通过当前拓扑继承。

相机同步：

- 选择 DataSource 并输入源库 `cameras.id`。
- 从源库 `cameras` 表读取设备序列号、名称、通道和视频信息，创建或更新系统相机资产。
- 相机同步不在同步阶段分配 Workspace。

全量同步：

- 选择 DataSource 后读取源库 `devices`、`gate_node` 和 `cameras`。
- 根据 `devices.device_type` 识别标准站、网关和节点：小于 10 为标准站，等于 10 为网关，大于 10 为节点。
- 全量同步更新已有系统设备、数据流、外部映射、拓扑和相机，不覆盖平台维护的工作区业务资料。
...
- 节点不提供独立的工作区分配；节点可见性和权限由所属网关的有效分配决定。
- 管理员可以调整网关与节点拓扑，拓扑变化后节点权限随新的父网关生效。
