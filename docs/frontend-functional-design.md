...
- 展示模板由代码维护，后台不提供大屏模板管理、上下架、定价或手工同步入口。内置模板启动时自动注册；保留用户保存的视图、配置及其模板版本关联。

- 运行概况按当前视图设备范围统计设备总数、正常上报、上报超时和状态未知。以运行属性的最近采样时间为准：最近 24 小时为正常上报，超过 24 小时为超时；无记录、非法/未来时间、查询失败归为未知，不以启用状态或无告警推断健康，超时不等同于设备故障。状态批量查询、每分钟刷新。
- 全景模板的运行概况固定在左侧上方、设备列表位于其下，右侧为观测数据，两侧悬浮面板延伸至页面下缘；不再展示底部统计栏。

- `/atlas` 为模板与已保存视图入口，不自动进入默认地图。用户选择「科技总览」或「全景地图」，在自定义表单设置名称、设备范围、趋势和图片开关，保存后进入 `/atlas/:id`。取消不创建记录，保存失败保留表单输入并允许重试。
- 入口展示当前工作区已保存的有效视图，支持打开后继续自定义或另存为；无管理权限的成员可打开已保存视图，但不能创建或修改。仅在打开具体视图后加载观测数据与地图。

- `/atlas` 取代旧的大屏展示入口；`/atlas/:id` 打开保存的工作区视图。不迁移旧模板或旧播放页面。
- 深色科技主题采用自适应布局，不依赖固定 16:9 画布。具体视图为纯展示页面，顶部只保留全屏图标按钮，不展示模板选择、视图切换或自定义管理入口。全屏隐藏外层导航，可通过退出图标或 Escape 返回；不支持原生全屏的内嵌浏览器使用窗口内铺满。
- 自定义、另存为和归档入口位于 `/atlas` 已保存视图卡片的自定义表单中，沿用服务端管理权限。
- 展示模板分为「科技总览」和「全景地图」。后者以全幅地图为底，标题居中装饰，目录、数据和统计使用半透明悬浮面板；窄屏改为叠放面板保留访问能力。模板在入口的自定义表单选择并保存至 `config.presentation`，不改变设备和指标范围。
- 模板注册表 `atlas-templates.tsx` 管理模板标识、文案和布局组件。布局仅接收目录、地图、数据区插槽，查询、权限和配置保存在共享工作区层；新增模板须同步后端配置校验、OpenAPI 枚举和双语文案。
- 地图与设备目录联动，支持名称、序列号、类型过滤；无坐标设备仍可选中并查看数据。地图标记和启用状态不等同于设备在线状态。
- 所选设备展示完整指标目录；网关节点从统一 nodes 接口解析，支持 THCPN 子设备及 LoRaWAN V2 虚拟节点。指标按需查询，提供区间最新值、时间、独立单位趋势、统计和数值表。历史范围最长 30 天，采样和不完整数据必须明确标注。
- 采集图片按图片流和时间范围分页浏览并放大查看；资产照片单独展示。图片使用现有签名预览地址与媒体权限，不增加公开访问。
- 用户可保存标题、简介、设备范围、优先指标、默认时间范围、布局预设及图片/趋势模块显隐；未选设备表示工作区可访问全部设备。配置复用 wallboard 存储和 view/manage 权限，模板为 atlas-tech。
- 显示加载、错误、空数据和页面更新时间；查询键包含工作区、设备、指标和筛选条件。工作区切换时清空当前选中状态，不沿用其他工作区数据。
- 删除视图采用归档，只移除展示配置，不删除设备、指标或图片。
