...
- 只有 `is_system_admin=true` 的用户可以进入 `/admin`。
...
| 路由 | 页面职责 |
| --- | --- |
| `/admin` | 系统设备、Workspace、Project、Site 和 DataSource 的汇总入口 |
| `/admin/sources` | DataSource 列表、新建/修改、标准站同步和组网站同步 |
| `/admin/devices` | 系统设备资产、搜索筛选、拓扑、配置、生命周期、能力、相机和分配管理 |
| `/admin/metadata` | 设备能力定义和系统角色元数据管理 |
...
- 成功后保存 token，并根据 `is_system_admin` 进入后台或总览。
...
| `devices` | 普通设备查询、子节点、解绑、设备操作及后台资产管理 |
